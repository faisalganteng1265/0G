...
float pattern(vec2 p) {
  vec2 drift = vec2(time * waveSpeed, time * waveSpeed * 0.37);
  vec2 p2 = p + drift;
  return fbm(p2 + fbm(p2 + drift.yx));
}
...
void main() {
  ...
  vec2 animatedUv = uv + vec2(time * waveSpeed * 0.35, sin(time * waveSpeed) * 0.12);
  float f = pattern(animatedUv);
...
function DitheredWaves({
  ...
  const waveUniforms = useMemo<WaveUniforms>(
    ...
    // Three uniforms are mutable render resources; prop updates are applied in useFrame below.
  ...
  useFrame((_, delta) => {
    ...
    if (material) {
      material.uniforms.time.value = uniforms.time.value;
      material.uniforms.waveSpeed.value = uniforms.waveSpeed.value;
      material.uniforms.waveFrequency.value = uniforms.waveFrequency.value;
      material.uniforms.waveAmplitude.value = uniforms.waveAmplitude.value;
      material.uniforms.waveColor.value = uniforms.waveColor.value;
      material.uniforms.mousePos.value = uniforms.mousePos.value;
      material.uniforms.enableMouseInteraction.value = uniforms.enableMouseInteraction.value;
      material.uniforms.mouseRadius.value = uniforms.mouseRadius.value;
